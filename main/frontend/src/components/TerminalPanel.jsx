...
export default function TerminalPanel({ terminal, explanation, loading, onExportAPK, dark }) {
  ...
  return (
    <div className={`w-1/3 flex flex-col ${dark ? "bg-black" : "bg-gray-50"}`}>
      <div className={`p-2 border-b text-xs font-mono uppercase ${dark ? "border-gray-800 text-gray-500" : "border-gray-300 text-gray-400"}`}>
        Terminal Output
      </div>
      ...
      <div className={`p-4 border-t ${dark ? "border-gray-800" : "border-gray-300"}`}>
        <button
          ...
          className="w-full bg-gray-800 hover:bg-gray-700 text-white py-2 rounded flex items-center justify-center gap-2 disabled:opacity-50"
        >
